defmodule DailyTwang.Accounts.AccountToken do
  use Ecto.Schema
  import Ecto.Query
  ...
  schema "accounts_tokens" do
    ...
    belongs_to :account, DailyTwang.Accounts.Account
  ...
  defp build_hashed_token(account, context, sent_to) do
    ...
    {Base.url_encode64(token, padding: false),
     %DailyTwang.Accounts.AccountToken{
       token: hashed_token,
       context: context,
       sent_to: sent_to,
  ...
  def token_and_context_query(token, context) do
    from DailyTwang.Accounts.AccountToken, where: [token: ^token, context: ^context]
  end
  ...
  def account_and_contexts_query(account, :all) do
    from t in DailyTwang.Accounts.AccountToken, where: t.account_id == ^account.id
  end
